...
**Headline result:** a clean, replicated domain split, not a general answer. In code, self-judgment shows excess self-confirmation on four independent statistics. In math, none of the same five statistics confirms it, and two are significantly the *wrong* sign. Full results, method, and a complete account of pre-registration deviations are in the paper (`paper/main.md`, `paper/draft_v1.pdf`).
...
paper/
  ...
  figures/             All 6 figures referenced in the paper
code/
  ...
  make_findings_figures.py     Regenerates the three real-data results figures (Figures 4-6)
...
# Regenerate the three real-data figures (Figures 4-6)
cd code && python3 make_findings_figures.py
```

Re-running the real-model harness itself (`run_stage2_esc.py`) requires MLX on Apple Silicon, the cached `mlx-community/Qwen3.5-4B-4bit` checkpoint, and — for the `J_strong`/`J_strong~` judge conditions — either a local Ollama server (`gemma4:12b`, the default) or an OpenRouter API key (`OPENROUTER_API_KEY`) for the Claude Haiku / DeepSeek cross-checks.
