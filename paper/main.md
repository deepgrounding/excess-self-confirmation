...
The central limitation is the boundary of the attribution claim: the domain split is well controlled *within this design*, but it is not yet a universal property of code, math, or self-judging models. The full judge matrix was run only for Qwen3.5-4B on HumanEval+MBPP and GSM8K+MATH. The Llama-3.2-3B arm establishes that raw excess under a self-judge is not Qwen-specific, but because it lacks the noise-matched and placebo controls, it cannot determine whether that excess comes from self-preference or generic revision-conditioning. Generalization across model families, scales, and domains therefore remains open. Benchmark contamination is also plausible: within-model contrasts remove a constant contamination effect, but not an interaction between memorization and judge condition. The present result should consequently be read as a falsification of the *domain-invariant* account of self-confirmation, not as a final taxonomy of which domains are vulnerable.

The second limitation concerns replication and statistical dependence. The independent seed covers the pre-registered critical cells — $J_{\mathrm{self}}$, $J_{\mathrm{strong}\sim}$, and $N{=}1$ — rather than the entire judge matrix; $J_{\mathrm{peer}}$, $J_{\mathrm{oracle}}$, and $J_{\mathrm{placebo}}$ remain single-seed. Moreover, two seeds rule out a single-draw explanation but do not estimate the full across-seed sampling distribution. This matters most for $\mathrm{ESC}_{\mathrm{adj}}$: math's value of $-0.211$ is the largest-magnitude result in §7, yet its placebo baseline was not repeated. The caution is empirical rather than ceremonial, because the closely related $N{=}1$ estimate of revision-conditioning drift was the only component that changed significance across seeds (§7.8). Thus the *magnitude* of placebo-adjusted ESC is not established, although the underlying attribution result for math does not depend on it alone: the noise-matched $\mathrm{ESC}_{\mathrm{sp}}$ replicated and became more decisively negative. Repeating the two placebo arms at a second seed is the cheapest high-value extension (approximately two days on the hardware used here). More broadly, the five reported statistics share trajectories and are complementary controls, not independent replications; §10.3 states the resulting multiplicity constraint explicitly.

The third limitation is that the study identifies excess confirmation but does not yet map its response to stronger optimization or demonstrate the proposed early warning on real collapse. On synthetic data, raw SCG rises with candidate count while ESC remains stable (§5.4, Check 3), but the real-model $N \in \{8,12\}$ arms were not run. Likewise, H3's within-condition warning statistic passes a synthetic ground-truth test, not a real collapse test. Establishing that claim requires the deferred high-selection-pressure 4B cell [C] and 9B scale check [D], followed by the diversity overlay specified in §5.3. These experiments are the necessary next step before ESC can be presented as an operational early-stop signal rather than a validated measurement instrument.

The limitations therefore define a concrete progression rather than an open-ended agenda: first replicate the placebo baseline, then complete the cross-model attribution matrix, and finally test the $N$-response and H3 warning protocol under induced collapse. Each step maps to a designed and budgeted cell in §6.1, and the released harness, frozen pools, and analysis code make those tests continuations of the present design rather than new post-hoc studies.
...
**Excess self-confirmation is not an inherent property of self-judgment; in these experiments, it is a task-dependent failure mode that a raw score gap can both invent and conceal.** The decomposition $\mathrm{raw\,SCG}=\Delta\mathrm{SEL}+\mathrm{ESC}$ removes the optimizer's curse from the reported–true improvement gap and exposes two qualitatively different regimes. In code, self-preference produces a positive excess that survives noise-matched and placebo controls, even though cancellation between two significant terms makes the raw gap look nearly harmless. In math, the same controls attribute no excess to self-judgment: the apparent drift is consistent with generic revision-conditioning, and the self-judge is better calibrated than a content-free baseline after that inflation is removed — a direction that survives every control we ran, though its magnitude rests on a single-seed placebo arm (§9). The critical comparisons reproduce at an independent seed.

The broader methodological result is that controls in self-evaluation studies must themselves be measured, not assumed. Three unrelated strong judges reach the same score-saturation barrier on math, showing that external evaluation is not automatically an informative counterfactual. Conversely, revision-conditioning drift changes enough across seeds to make a no-selection or placebo baseline unstable. The four comparisons in this paper are therefore valuable because they triangulate one decomposition under different controls, not because they provide four statistically independent confirmations.

For deployed self-improvement loops, the implication is direct: monitoring only the judge's selected score is insufficient, and even monitoring a raw reported–true gap can diagnose the wrong mechanism. A useful audit must separate selection inflation, pool-wide optimism drift, and judge-identity-specific preference, then validate those components in the target domain. On the present evidence, the right question is not whether self-judgment is safe in general, but **which mechanism is producing apparent progress in this task, with this judge, under this selection pressure**.
