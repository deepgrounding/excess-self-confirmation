...
When a language model generates candidates, judges them, and revises under its own scores, reported progress can outrun true progress. Most of that gap is not self-deception: it is the optimizer's curse — argmax over a noisy judge inflates the selected score while true quality is unaffected. We decompose the raw reported-true gap into a per-round selection term (SEL, the optimizer's curse) and a residual, $\mathrm{ESC}$ ("excess self-confirmation"), algebraically identical to the drift of the *candidate pool's* optimism bias, and ask on a real closed loop (Qwen3.5-4B, MLX, HumanEval+MBPP and GSM8K+MATH, $N{=}4$, $T{=}15$, 84–91 items/domain) whether $\mathrm{ESC}$ is attributable to self-preference or is fully explained by generic effects any judge would show. **We find a clean, replicated domain split, not a general answer.** In code, self-judgment shows excess self-confirmation on **four independent statistics**: $\mathrm{ESC}(T){=}+0.115$ ($p{=}0.0013$), its cumulative form $\mathrm{AUC}_{\mathrm{ESC}}{=}+1.76$ ($p{=}0.0003$), a noise-matched differential against an external judge $\mathrm{ESC}_{\mathrm{sp}}{=}+0.129$ ($p{=}0.012$), and a placebo-adjusted differential $\mathrm{ESC}_{\mathrm{adj}}{=}+0.177$ ($p{=}0.002$) — the raw self-confirmation gap in code is a near-perfect, coincidental cancellation of two *individually significant* opposite-signed terms ($\Delta\mathrm{SEL}{=}-0.114$, $\mathrm{ESC}{=}+0.115$), which a monitor tracking only the raw gap would completely miss. In math, none of the same five statistics (the four above plus an independent $N{=}1$ pure-revision path) confirms excess self-confirmation, and two — the noise-matched differential and the placebo-adjusted differential — are *significantly the wrong sign*: math's self-judge shows measurably *less* apparent excess than a content-free placebo judge, once generic revision-conditioning inflation is subtracted out. A structural methodological finding accompanies this: three architecturally unrelated strong judges (a 12B local model, a frontier API model, and a reasoning-tuned MoE model) all independently fail the same AUC-vs-winner's-curse noise-matching gate on math, converging on nearly the same operating point — evidence that math's external-judge score saturation is a property of the grading signal, not an artifact of any one judge. We release the full harness, candidate-pool trajectory library, and analysis code so every number here recomputes offline, and pre-register the design elements that were frozen before data collection (git tags `prereg-opt2-v2`, `prereg-opt2-v2-noise`) alongside a full account of post-freeze amendments.
...
```text
ESC_sp(t) = ESC_self(t) - ESC_strong~(t)
```
...
Model family substitution from the original proposal draft (Qwen2.5-Instruct) to Qwen3.5, matching the companion registered report [@chen2026error] under the same size-class-preserving, non-thinking-mode contingency, changed no hypothesis or grid — only the checkpoint family [@qwen2026qwen35omni; @yang2025qwen3].
...
**Cumulative statistic.** $\mathrm{AUC}_{\mathrm{ESC}} = \int_0^T \mathrm{ESC}(t)\,dt$ (trapezoidal), reported alongside $\mathrm{ESC}(T)$ in §7 as a second, largely independent read of the same trajectory (significant across the whole path, not only at the terminal round, or vice versa).
...
**Check 3: decomposition separates selection from drift (Figure 1).** Under a *null* regime with true ESC $= 0$ (stationary pool, no injected drift):

| $N$ | SEL level | ESC$(T)$ (this design) | raw SCG$(T)$ (v1 protocol: unselected round 0) |
|---:|---:|---:|---:|
| 1 | 0.000 | 0.009 | 0.009 |
| 4 | 0.113 | 0.000 | 0.115 |
| 8 | 0.150 | $-0.001$ | 0.153 |
...
| [B] $N$-response curve | 4B $\times$ code $\times$ $J_{\mathrm{self}}$ $\times$ $N \in \{1,8,12\}$ ($N{=}1$ also math) | **$N{=}1$ only, both domains** (independent H1 path, §7); $N \in \{8,12\}$ not run |
...
Second-seed replication on critical cells was not run (§9). §8 explains why the scope was reduced from the full A–E grid to cell [A] plus [B]'s $N{=}1$ arm before this paper's results were finalized.
...
### 7.3 $\mathrm{AUC}_{\mathrm{ESC}}$ and the independent $N{=}1$ path
...
$\mathrm{AUC}_{\mathrm{ESC}}$ reinforces rather than revises §7.2: code's cumulative drift is significantly positive across the whole trajectory, not only at the terminal round; math's is directionally positive but its CI still spans zero. Cell [B]'s independent $N{=}1$ path does *not* confirm H1 in either domain — both CIs span zero, and math's point estimate is the wrong sign. At this point (two statistics per domain): **code shows excess self-confirmation on two independent statistics; math clears none of the three computed so far.**
...
| Statistic | Code | Math |
|---|---|---|
| $\mathrm{ESC}(T)$ | $+0.115$, $p{=}0.0013$ — **confirms** | $+0.025$, $p{=}0.235$ — n.s. |
| $\mathrm{AUC}_{\mathrm{ESC}}$ | $+1.757$, $p{=}0.0003$ — **confirms** | $+0.023$, $p{=}0.481$ — n.s. |
| Cell [B] $N{=}1$ SCG$(T)$ | $+0.028$, $p{=}0.334$ — n.s. | $-0.024$, $p{=}0.685$ — n.s., wrong sign |
| $\mathrm{ESC}_{\mathrm{sp}}(T)$ vs. $J_{\mathrm{strong}\sim}$ | $+0.129$, $p{=}0.012$ — **confirms** | $-0.072$ to $-0.089$, $p{\ge}0.93$ — wrong sign |
| $\mathrm{ESC}_{\mathrm{adj}}(T)$ vs. $J_{\mathrm{placebo}}$ | $+0.177$, $p{=}0.0016$ — **confirms** | $-0.211$, $p{=}0.9998$ — **significantly wrong sign** |

Code confirms on **4 of 5** statistics (missing only the underpowered $N{=}1$ path); math confirms on **0 of 5**, with **2 of 5 significantly the wrong sign**. This is not a power difference between domains — the two statistics that resolve math most clearly ($\mathrm{ESC}_{\mathrm{sp}}$, $\mathrm{ESC}_{\mathrm{adj}}$) are themselves independently significant, just in the opposite direction from H1/H2's prediction. **The mechanism this paper set out to isolate — excess, judge-identity-specific self-confirmation — is present and repeatedly confirmed in code, and repeatedly, consistently absent in math.**
...
This paper's design — hypotheses (§3), method (§4), metrics (§5), the synthetic validation (§5.4), the pre-registered cell list, gates, and analysis plan (§6, `ANALYSIS_PLAN.md`) — was frozen under git tag **`prereg-opt2-v2`** before any real-model data collection began, following the registered-report model [@chambers2013registered; @nosek2018preregistration]. The noise-matching $(\tau,\lambda)$ calibration (§7.5) could not exist before that data was collected; it was frozen under a follow-up amendment tag **`prereg-opt2-v2-noise`**. Both tags ship with this paper's public repository and give the design freeze an externally verifiable timestamp, in place of the separate arXiv Stage-1 submission originally planned (below).
...
4. **Merged the originally-planned two-part structure (a Stage 1 registered-report design submission, followed by a separate Stage 2 results companion) into this single paper.** An initial Stage-1-only arXiv submission was made and subsequently placed on moderation hold; rather than pursue it, we withdrew it and report design, deviations, and results together here, anchored by the git tags above rather than a separate design-only submission. No results were altered by this decision — items 1–3 above were already finalized before it was made.

None of these deviations change what H1/H2 predict or how they are tested; all are disclosed with the exact point in the pipeline at which they occurred, and the full commit and tag history is public. We regard git-tag-anchored preregistration with an open deviation log, published alongside results, as at least as verifiable as a separate design-only submission, and considerably more useful to a reader who wants both the design and the answer in one place.
...
**Second-seed replication was not run.** All real-model results in §7 are single-seed (seed 13). The critical-cell replication plan ($J_{\mathrm{self}}$, $J_{\mathrm{strong}\sim}$, $N{=}1$) specified in the original design was not executed within this paper's scope.

**Two domains, one model scale.** All real-model results are Qwen3.5-4B on HumanEval+MBPP and GSM8K+MATH. Generalization to other model families, scales (a 0.8B harness pilot exists, §10.1, but is not H1–H3 evidence at $n{=}7$), or task domains beyond code and math is untested. The domain split we report (self-preference-driven in code, absent in math) is a real, four-way-replicated finding *within this design*; whether "code" and "math" generalize to other verifiable-task domains, or whether the split tracks something more specific (e.g., external-judge score saturation, which we do observe is domain-specific in §7.5) is open.
...
- Freeze order: item screen and bins $\to$ prompts/rubrics $\to$ noise calibration $\to$ `prereg-opt2-v2`/`prereg-opt2-v2-noise` tags $\to$ main arm $\to$ analysis unblinding.
- Code sandbox: subprocess isolation, 10 s timeout, 512 MB, no network.
- One-command recomputation: all metrics and figures derive from the pool library; `code/analyze_cell_A_h1.py` reproduces every number in §7, `code/sim_validate.py` and `code/make_validation_figures.py` reproduce §5.4 without GPU or LLM access.
